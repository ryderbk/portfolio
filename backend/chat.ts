import { getChatResponse } from '../src/services/ai.js';

export async function chatHandler(req: any, res: any) {
  try {
    const { message, context, history } = req.body;

    if (!message) {
      return res.status(400).json({ reply: "Message is required." });
    }

    // Use context if provided, otherwise fallback to empty
    const portfolioContext = context || { projects: [], skills: [], experiences: [] };
    
    // Pass conversation history for multi-turn context
    const conversationHistory = Array.isArray(history) ? history : [];
    
    const reply = await getChatResponse(message, portfolioContext, conversationHistory);
    return res.status(200).json({ reply });

  } catch (error: any) {
    console.error("BACKEND CHAT ERROR:", error.message || error);
    return res.status(500).json({
      reply: "I apologize, but the AI assistant is temporarily unavailable. Please try again shortly."
    });
  }
}
