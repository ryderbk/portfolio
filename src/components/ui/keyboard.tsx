...
const KEY_CODE_TO_CHAR: Record<string, string> = {
...
interface KeyboardContextType {
  ...
  setPressed: (keyCode: string, isVirtual?: boolean) => void;
  setReleased: (keyCode: string) => void;
  lastPressedKey: string | null;
  onKeyPress?: (key: string, isVirtual: boolean) => void;
}
...
const KeyboardProvider = ({
  ...
  const setPressed = useCallback((keyCode: string, isVirtual: boolean = false) => {
    setPressedKeys((prev) => new Set(prev).add(keyCode));
    setLastPressedKey(keyCode);
    if (onKeyPress && KEY_CODE_TO_CHAR[keyCode]) {
      onKeyPress(KEY_CODE_TO_CHAR[keyCode], isVirtual);
    }
  }, [onKeyPress]);
...
const KeystrokePreview = () => {
  ...
  return (
    <div className="relative flex h-24 w-full items-center justify-center -mb-8 z-50 pointer-events-none">
      <AnimatePresence mode="popLayout">
        {displayKey && (
          <motion.div
            ...
            exit={{ opacity: 0, scale: 1.5, y: -20, filter: "blur(20px)" }}
            transition={{
              type: "spring",
              stiffness: 400,
              damping: 25,
          ...
          >
            <span className="font-mono text-6xl font-black text-primary/90 drop-shadow-[0_0_20px_rgba(var(--primary-rgb),0.5)] uppercase tracking-tighter">
              {displayKey}
            </span>
          </motion.div>
        )}
...
export const Keyboard = ({
  ...
  return (
    <KeyboardProvider enableSound={enableSound} containerRef={containerRef} onKeyPress={onKeyPress}>
      <div
        ref={containerRef}
        className={cn(
          "mx-auto w-fit [zoom:0.7] sm:[zoom:1.2] md:[zoom:1.5] lg:[zoom:1.8] xl:[zoom:2.2]",
          className,
        )}
      >
...
export const Keypad = () => {
  return (
    <div className="h-full w-fit rounded-xl bg-muted p-1 shadow-sm ring-1 shadow-black/5 ring-border/50">
      {/* Function Row */}
      <Row>
        <Key
          keyCode="Escape"
          ...
          childrenClassName="items-start justify-center pl-[4px]"
        >
          <span>esc</span>
        </Key>
        <Key keyCode="F1">
      ...
      <Row>
        ...
        <Key
          keyCode="Backspace"
          className="w-10"
          childrenClassName="items-end justify-center pr-[4px]"
        >
          <span>delete</span>
        </Key>
      </Row>
  ...
  return (
    <div className={cn("rounded-[4px] p-[0.5px]", containerClassName)}>
      <button
        ...
        className={cn(
          "flex h-6 w-6 cursor-pointer items-center justify-center rounded-[3.5px] bg-card border border-border shadow-[0px_1px_1px_0px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(255,255,255,0.1)_inset] transition-transform duration-75 active:scale-[0.98]",
          isPressed &&
            "scale-[0.98] bg-primary text-primary-foreground shadow-inner",
          className,
        )}
      >
        <div
          className={cn(
            "flex h-full w-full flex-col items-center justify-center gap-[2px] font-bold leading-none uppercase whitespace-nowrap",
            isPressed ? "text-primary-foreground" : "text-foreground/90",
            childrenClassName,
          )}
...
const ModifierKey = ({
  ...
  return (
    <div className={cn("rounded-[4px] p-[0.5px]", containerClassName)}>
      ...
      >
        <div className={cn(
          "flex h-full w-full flex-col items-start justify-between px-[2px] py-[3px] font-bold leading-none uppercase whitespace-nowrap",
          isPressed ? "text-primary-foreground" : "text-foreground/90"
